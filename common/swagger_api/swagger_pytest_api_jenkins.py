            ...
            for method, values in api_dicts.items():
                ...
                if list(self._parse_parameters(values.get('parameters', [])).values()):
                    ...
                    if if_params_in_url:
                        ...
                        # path = path.replace(url_params, '')
                        # path = url_params + '.format({})'.format(url_params)
                        list_params.append(url_params)
                    if list_params:
                        list_params = set(list_params) # 去重
                        params = ""
                ...
                name = name.replace("/", "_").replace("-", "_").replace("{", "").replace("}", "")[1:]
                if if_params_in_url:
                    url_params = path[path.find('{') + 1:path.find('}')]
                    path = path.replace(url_params, '')
                    path = path + '.format({})'.format(url_params)
                self.result.append(
                    {
...
if __name__ == '__main__':
    swagger_url = os.getenv('swagger_url', 'http://172.16.204.29:8044/v2/api-docs?group=v1')
    class_name = os.getenv('class_name', 'test')
    # to_login_names = os.getenv('to_login_names', 'lvjunjie')

    # swagger_url = 'http://mdp.test.zghbh.com/v2/api-docs'
    # class_name = 'hunbohuiMDP'
    # object_file_name = '../apis/huibohuiMDP3.py'
